...
class RotateX : public Mesh {
    public:
        RotateX( shared_ptr< Mesh > mesh, double angle ) : mesh( mesh ){
            auto radians = degreesToRadians( angle );
            ...
            for( int i = 0; i < 2; i++ ){
                for( int j = 0; j < 2; j++ ){
                    for( int k = 0; k < 2; k++ ){
                        ...
                        auto newX = cosTheta * y + sinTheta * z;
                        auto newZ = -sinTheta * y + cosTheta * z;

                        Vector3 coordinates( x, newX, newZ );
        ...
        bool hit( const Ray &ray, Interval interval, IntersectionManager &intersectionManager ) const override {
            ...
            intersectionManager.point = Point3(
                intersectionManager.point.x(),
                ( cosTheta * intersectionManager.point.y() ) + ( sinTheta * intersectionManager.point.z() ),
                ( -sinTheta * intersectionManager.point.y() ) + ( cosTheta * intersectionManager.point.z() )
            );

            intersectionManager.normal = Vector3(
                intersectionManager.normal.x(),
                ( cosTheta * intersectionManager.normal.y() ) + ( sinTheta * intersectionManager.normal.z() ),
                ( -sinTheta * intersectionManager.normal.y() ) + ( cosTheta * intersectionManager.normal.z() )
            );
